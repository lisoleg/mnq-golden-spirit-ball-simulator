...
MNQ 金灵球网络仿真器是对 macOS 原生 MNQ 项目 (Swift/C/Metal) 的完整 Windows 移植与升级，实现了复合体理学微信公众号 14 篇文章中描述的全部核心理念。v2.0 新增三层信息波自洽场 (SCF)、CGD 约束生成动力学、MNQ9 信心核模型与反馈回路，提供 12 项预定义实验的 CLI/GUI 双模式操作。
...
### 仪表盘 (`mnq_dashboard.py` v2.0, ~650行)

- **GUI 模式**: tkinter 多面板仪表盘 (SCF/CGD/MNQ9/金灵球/Hex64)
- **CLI 模式**: 12 项预定义实验，一键批量运行
...
```
mnq_windows/
├── mnq_core.py           # 核心引擎 v2.0 (~1050行)
├── mnq9_core.py          # MNQ9 信心核 (~300行)
├── mnq_dashboard.py      # GUI仪表盘 + CLI v2.0 (~650行)
